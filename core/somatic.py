...
def _is_typed_user_text(s: str) -> bool:
    """Heuristic: True if text looks like real human typing, not pasted skill
    bodies, system reminders, or context summaries.

    False positives in valence detection come from skill bodies and reminder
    blocks containing keywords like ``wrong``/``fail``/``error`` inside their
    documentation. This filter rejects those before pattern matching.
    """
    s = s.strip()
    if not s:
        return False
