...
- [x] **FOUND-02**: Wire user prompt text into Tier 2 injection path (currently context-free, only project match)
- [x] **FOUND-03**: Implement Thompson sampling for memory selection using Beta(usage_count+1, unused_count+1) from stdlib
- [x] **FOUND-04**: Add provenance signals at injection time — "established across N sessions over M weeks" metadata in injection format
