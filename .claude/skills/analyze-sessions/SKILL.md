...
<MANDATORY>
Read the signal summary. Before checking what the pipeline captured, form your own list. Forming expectations after seeing actual output contaminates the audit with hindsight bias.
</MANDATORY>
...
| `parse_error_repaired` | max_tokens truncation hit |
...
print(f'Observations in DB: {len(obs)}')
for o in obs:
    print(f'  [{getattr(o, \"kind\", \"?\")}] {str(o.content)[:120]}')
close_db()
"
```
...
| Tier 1 | Session-type guidance; skip friction; low-affect prefilter; schema field promotion | 1-week tactical |
